...
if (UOMA_RATE_LIMIT) {
    ...
    if ($redis->connect(UOMA_REDIS['host'], UOMA_REDIS['port'])) {
        ...
        $apiKey = 'refresh-mail-list-'.$user['email'];
...
$cached_response = get_cache('mail_refresh', $user['email'], $conn);
...
if ($cached_response !== false) {
    ...
    // Fetch emails within a day
    $all = imap_search($imap, 'UNDELETED SINCE "'.date('j-M-Y', strtotime('-1 day')).'"');
    ...
    // Set cache
    set_cache('mail_refresh', $user['email'], $headers, '+1 minute', $conn);

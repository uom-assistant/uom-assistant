...
if ($cached_response !== false) {
    ...
    $active_href = $crawler->filter('.container')->first()->filter('ul.nav.nav-tabs > li.nav-item.active > a')->attr('href');

    $grade_list_all_crawler = $crawler->filter('.container')->first()->filter('ul.nav.nav-tabs > li.nav-item')->each(function(\Symfony\Component\DomCrawler\Crawler $node, $i) {
        $ele = $node->filter('a');
    ...
    // Read grades
    foreach ($grade_list_all_crawler as $item) {
        $grade_list = $crawler->filter($item['href'])->first()->filter('.container > table.cwk')->each(function(\Symfony\Component\DomCrawler\Crawler $node, $i) {
